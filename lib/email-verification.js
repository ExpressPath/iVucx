...
const RATE_LIMIT = 5;
const DEFAULT_MAX_SEND_RATE_BUCKETS = 2000;
...
function consumeSendRateLimit(req, email) {
  ...
  const maxBuckets = Math.max(
    500,
    Math.floor(Number(process.env.IVUCX_EMAIL_RATE_BUCKETS_MAX) || DEFAULT_MAX_SEND_RATE_BUCKETS)
  );
  if (!sendRateBucket.has(key) && sendRateBucket.size >= maxBuckets) {
    let evictionKey = '';
    ...
    for (const [bucketKey, values] of sendRateBucket.entries()) {
      ...
      if (!nextValues.length) {
        sendRateBucket.delete(bucketKey);
        continue;
      }
      sendRateBucket.set(bucketKey, nextValues);
      const firstRequest = Number(nextValues[0]) || 0;
      if (firstRequest < oldestRequest) {
        evictionKey = bucketKey;
        oldestRequest = firstRequest;
      }
    }
    if (sendRateBucket.size >= maxBuckets && evictionKey) {
      sendRateBucket.delete(evictionKey);
    }
  }
