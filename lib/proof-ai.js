...
const PROVIDER_TIMEOUT_MS = Math.max(5000, Number(process.env.PROOF_AI_PROVIDER_TIMEOUT_MS) || 90000);
const PROVIDER_RESPONSE_MAX_BYTES = Math.max(65536, Number(process.env.PROOF_AI_RESPONSE_MAX_BYTES) || 1024 * 1024);
...
async function getProofAiAccountIdentity(req, supabase) {
  return getIvucxIdentity(req, supabase);
}
...
async function registerProofAiProvider(req, res, body) {
  ...
  const model = boundedString(body.model, 160, config.model);
  const keySource = boundedString(body.keySource, 64, body.hasLocalKey ? 'browser-session' : 'server-env');
  const now = utcNowIso();
...
async function fetchExternalProofContext({ body, language, contextEngine }) {
  ...
  try {
    ...
    const responseText = await readBoundedResponseText(response, MAX_EXTERNAL_CONTEXT_CHARS * 4);
...
async function callOpenAiCompatible({ config, apiKey, model, messages }) {
  const { response, payload } = await requestProviderJson(joinUrl(config.baseUrl, config.endpoint), {
...
async function requestProviderJson(url, headers, body) {
  const controller = new AbortController();
  ...
  try {
    ...
    const text = await readBoundedResponseText(response, PROVIDER_RESPONSE_MAX_BYTES);
    let payload = null;
    try {
      payload = text ? JSON.parse(text) : null;
    } catch (error) {
      if (response.ok) {
        const malformed = new Error('AI provider returned an invalid response.');
...
export async function sendProofAiResponse(req, res) {
  ...
  try {
    ...
    const contextEngine = normalizeContextEngine(body.contextEngine, language);
    const config = providerConfig(provider);
    const suppliedApiKey = safeString(body.apiKey);
    if (suppliedApiKey.length > 4096) {
      res.status(400).json({ error: 'API key is too long.' });
      return;
    }
    const apiKey = suppliedApiKey || config.apiKey;
    const model = boundedString(body.model, 160, config.model);
    ...
    res.status(status).json({
      error: getPublicErrorMessage(error, 'AI request failed.', status)
    });
  }
}
