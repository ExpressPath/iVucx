...
function buildProblemRecord(input, result) {
  ...
  const proofState = normalizeProofState(result.proofState || result.proof_state || input.proofState || input.proof_state)
    || inferProofState(language, code, result);
  const jobId = String(input.jobId || input.helperJobId || '').trim();
  const problemKind = String(input.problemKind || input.postKind || input.searchKind || '').trim().toLowerCase();
  const bounty = problemKind === 'problem' && proofState === 'NY'
    ? normalizeBounty(input.bounty)
    : null;
  ...
  return {
    record: {
      ...
      request_meta: {
        ...
        requestedFormat,
        completedFormat,
        problemKind: problemKind === 'problem' || problemKind === 'theorem' ? problemKind : null,
        bounty
      }
    },
    jobId
