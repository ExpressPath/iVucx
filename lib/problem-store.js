...
function buildProblemRecord(input, result, identity = null) {
  ...
  const bounty = problemKind === 'problem' && proofState === 'NY'
    ...
    : null;
  const conditionalBounty = problemKind === 'conditional'
    ? normalizeConditionalBounty(input.conditionalBounty || input.bounty)
    : null;
...
export async function persistHelperProblem(body, req = null) {
  ...
  const { record, jobId } = buildProblemRecord(input, result, identity);
