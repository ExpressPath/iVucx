...
function extractSupabaseProjectRefFromUrl(value) {
  ...
  try {
    const parsed = new URL(raw);
    const host = parsed.hostname || '';
    const apiHostMatch = host.match(/^([a-z0-9-]+)\.supabase\.co$/i);
    if (apiHostMatch) {
      return normalizeSupabaseProjectRef(apiHostMatch[1]);
    }
    const directHostMatch = host.match(/^db\.([a-z0-9-]+)\.supabase\.co$/i);
    if (directHostMatch) {
      return normalizeSupabaseProjectRef(directHostMatch[1]);
    }
  } catch (error) {
...
function summarizeJwt(token) {
  ...
  return {
    ...
    ref: normalizeSupabaseProjectRef(payload && payload.ref),
...
export function getSupabaseAdminDiagnostics() {
  const { url, urlSource, serviceRoleKey, anonKey } = resolveSupabaseAdminEnv();
  const serviceRole = summarizeJwt(serviceRoleKey);
  const anon = summarizeJwt(anonKey);
  const urlRef = extractSupabaseProjectRefFromUrl(url);
  const refs = [urlRef, serviceRole.ref, anon.ref].filter(Boolean);
  ...
  return {
    ...
    urlSource,
    projectRef: urlRef || serviceRole.ref || anon.ref || '',
    serviceRole,
    anon,
    refMismatch: uniqueRefs.length > 1
  };
}
