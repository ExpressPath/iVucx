...
export async function getIvucxAccountSnapshot(req) {
  ...
  try {
    ...
    const transactions = await listTransactions(supabase, identity);
    return {
      loggedIn: true,
      identity,
...
async function loadProblemRow(supabase, id, label) {
  ...
  const { data, error } = await supabase
    .from('problems')
    .select('id,title,language,file_name,source_code,source_sha256,proof_state,verification_status,verification_result,normalized_term,request_meta,created_at,updated_at')
    .eq('id', problemId)
    .maybeSingle();
  if (error) throw error;
  if (!data || !data.id) {
...
async function verifyStoredProblemBountyForPayout(originalMeta, originalProblemId) {
  ...
  const conditionals = Array.isArray(meta.conditionals)
    ...
    : [];
  for (const conditional of conditionals) {
    const conditionalBounty = normalizeConditionalBounty(conditional.bounty);
    if (!conditionalBounty || !conditionalBounty.stripeSessionId) continue;
    const verified = normalizeConditionalBounty(await verifyConditionalCheckoutSession(conditionalBounty, {
      problemId: originalProblemId,
      skipIdentityCheck: true
    }));
    const split = verified && verified.split ? verified.split : splitConditionalYen(verified ? verified.amountYen : 0);
    const problemBountyYen = Math.max(0, Math.round(Number(split.problemBountyYen) || 0));
    if (problemBountyYen <= 0) continue;
...
async function buildConditionalUsagePayouts({ supabase, originalProblem, solutionProblem, usedConditionals, shareYen, nowIso }) {
  ...
  const credits = [];
  const payouts = [];
  if (amountYen <= 0 || amountVx <= 0) return { credits, payouts };
