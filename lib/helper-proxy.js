...
function createRemoteConfigurationError(role) {
  ...
  const error = new Error(
    'Execution API is not configured on this server. Set EXECUTION_API_BASE_URL, EXECUTION_SERVER_BASE_URL, or ORACLE_SERVER_BASE_URL, or enable ALLOW_LOCAL_EXECUTION_FALLBACK for development.'
  );
  error.statusCode = 503;
  return error;
}
...
function resolveExecutionBridgeBaseUrl(req) {
  return normalizeBaseUrl(EXECUTION_API_BASE_URL || inferExecutionBaseUrlFromRequest(req));
}
...
function createLikelyWrongOracleEndpointError(baseUrl) {
  const error = new Error(
    `Execution API base URL looks like the Oracle Cloud control-plane endpoint (${baseUrl}). Point ORACLE_SERVER_BASE_URL to the public URL of the Oracle-server app that serves /api/lean-check, /api/coq-check, and /api/proof-convert.`
  );
  error.statusCode = 503;
  return error;
}
...
export async function proxyCompositeHelperInfo(req, res) {
  ...
  try {
    const effectiveExecutionBaseUrl = resolveExecutionBridgeBaseUrl(req);
    ...
    const payload = {
      ...upstream.payload,
      executionConfigured: Boolean(upstream.payload.executionConfigured || effectiveExecutionBaseUrl),
      capabilities: {
        ...(upstream.payload.capabilities || {}),
        proofCheck: true,
        splitExecution: Boolean(isExecutionConfigured() || effectiveExecutionBaseUrl),
        executionBridge: Boolean(effectiveExecutionBaseUrl)
      },
      deployment: {
        ...(upstream.payload.deployment || {}),
        helperServer: HELPER_API_BASE_URL || null,
        executionServer: effectiveExecutionBaseUrl || null,
        proofExecution: effectiveExecutionBaseUrl ? 'oracle-server-via-bridge' : 'same-app-fallback',
        conversion: 'railway-plan-to-oracle-server'
      }
    };
    ...
    payload.execution = {
      ...payload.execution,
      configured: Boolean(payload.execution.configured || effectiveExecutionBaseUrl),
      baseUrl: payload.execution.baseUrl || effectiveExecutionBaseUrl || null,
      via: payload.execution.baseUrl ? 'helper-env' : (effectiveExecutionBaseUrl ? 'request-bridge' : 'unconfigured')
    };
    ...
    payload.runtimes.proofRuntimes = {
      lean: {
        ...
        upstream: effectiveExecutionBaseUrl ? 'oracle-server-via-bridge' : 'same-app-fallback'
      },
      coq: {
        available: true,
        route: '/api/coq-check',
    ...
    payload.deployment.heavyConversion = payload.runtimes.executionModel
      && payload.runtimes.executionModel.conversion
      ? payload.runtimes.executionModel.conversion
      : (effectiveExecutionBaseUrl ? 'oracle-server-via-bridge' : 'same-app-fallback');
    ...
    if (isLikelyOracleControlPlaneUrl(effectiveExecutionBaseUrl)) {
      payload.deployment.executionWarning = {
        ...
        message: `Execution server base URL ${effectiveExecutionBaseUrl} looks like the Oracle Cloud control-plane endpoint, not the Oracle-server app URL.`
      };
    }
