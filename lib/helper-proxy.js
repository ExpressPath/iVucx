  ...
  try {
    ...
    const payload = {
      ...
      deployment: {
        ...
        proofExecution: effectiveExecutionBaseUrl ? 'render-via-helper-bridge' : 'same-app',
        conversion: 'railway-plan-to-render'
      }
    };
    ...
    payload.runtimes.proofRuntimes = {
      lean: {
        ...
        upstream: effectiveExecutionBaseUrl ? 'render-via-helper-bridge' : 'same-app'
      },
      coq: {
        available: true,
        route: '/api/coq-check',
    ...
    payload.deployment.heavyConversion = payload.runtimes.executionModel
      && payload.runtimes.executionModel.conversion
      ? payload.runtimes.executionModel.conversion
      : (effectiveExecutionBaseUrl ? 'render-via-helper-bridge' : 'same-app');
