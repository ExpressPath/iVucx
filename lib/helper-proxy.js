...
function createRemoteConfigurationError(role) {
  ...
  const error = new Error(
    'Execution API is not configured on this server. Set EXECUTION_API_BASE_URL (or ORACLE_SERVER_BASE_URL) or enable ALLOW_LOCAL_EXECUTION_FALLBACK for development.'
  );
  error.statusCode = 503;
  return error;
}
