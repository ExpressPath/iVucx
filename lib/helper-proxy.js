...
import { issueJobCapability } from './job-access.js';
import {
  normalizeRemoteServiceBaseUrl,
  tryNormalizeRemoteServiceBaseUrl
} from './remote-url-policy.js';
...
const MAX_REMOTE_RESPONSE_BYTES = Math.max(65536, Number(process.env.IVUCX_REMOTE_RESPONSE_MAX_BYTES || 4 * 1024 * 1024));
...
async function readBoundedResponseText(response, limit = MAX_REMOTE_RESPONSE_BYTES) {
  const contentLength = Number(response.headers.get('content-length') || 0);
  if (contentLength > limit) {
    const error = new Error('Remote service response exceeds the size limit.');
...
function normalizeBaseUrl(value) {
  return tryNormalizeRemoteServiceBaseUrl(value);
}
...
async function callAutoscaler(path, payload) {
  ...
  try {
    ...
    const text = await readBoundedResponseText(response, 512 * 1024);
    cleanupTimer(timer);
    if (!response.ok) {
...
async function sendRemoteRequest(role, targetPath, options = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    normalizeRemoteServiceBaseUrl(worker.baseUrl);
    worker.inflight += 1;
    const startedAt = Date.now();
    const { controller, timer } = createController(config.timeoutMs);

    try {
      const requestHeaders = role === 'execution'
        ? await attachExecutionRequestAuthHeaders({
            headers,
            method,
            targetPath,
            bodyText
...
function proxyErrorMessage(error, fallback) {
  ...
  if (NODE_ENV === 'production' && status >= 500) return fallback;
  return error && error.message ? error.message : fallback;
}

function sendRemotePayload(res, upstream, fallback) {
  if (NODE_ENV === 'production' && Number(upstream && upstream.status) >= 500) {
    res.status(upstream.status).json({ ok: false, error: fallback });
    return;
  }
...
export function sendRemoteConfigurationError(res, role = 'execution') {
  ...
  res.status(error.statusCode || 503).json({
    ...
    success: false,
    status: 'error',
    error: proxyErrorMessage(error, 'Proof service is temporarily unavailable.')
  });
}
...
export async function proxyHelperRequest(req, res, targetPath) {
  ...
  try {
    assertProofRequestAllowed(req, targetPath, {
      body: req.method !== 'GET' && req.method !== 'HEAD' ? (req.body || {}) : undefined
    });
    await assertDistributedRateLimit(req, {
      route: targetPath.includes('/jobs') ? 'proof-jobs' : 'proof-helper',
      limit: targetPath.includes('/jobs') ? 120 : 12,
      windowSeconds: 60
    });
    ...
    sendRemotePayload(res, upstream, 'Proof helper is temporarily unavailable.');
  } catch (error) {
    if (error.retryAfter) {
    ...
    res.status(error.statusCode || 502).json({
      ...
      error: proxyErrorMessage(error, 'Proof helper is temporarily unavailable.')
    });
  }
}
...
export async function proxyExecutionApiRequest(req, res, targetPath) {
  ...
  try {
    ...
    await assertDistributedRateLimit(req, { route: 'proof-execution', limit: 15, windowSeconds: 60 });
    await assertDistributedRateLimit(req, {
      route: 'proof-execution-global',
    ...
    const upstream = await sendRemoteRequest('execution', targetPath, {
      method: req.method,
      body: targetPath === '/api/proof-convert' && requestBody && typeof requestBody === 'object'
        ? { ...requestBody, verify: true }
        : requestBody
    });

    sendRemotePayload(res, upstream, 'Proof execution is temporarily unavailable.');
  } catch (error) {
    if (error.retryAfter) {
      res.setHeader('Retry-After', String(error.retryAfter));
    }
    res.status(error.statusCode || 502).json({
      ok: false,
      status: error.statusCode === 504 ? 'timeout' : 'error',
      error: proxyErrorMessage(error, 'Proof execution is temporarily unavailable.')
    });
  }
}
...
export async function proxyExecutionProofCheckRequest(req, res, targetPath) {
  ...
  try {
    ...
    sendRemotePayload(res, upstream, 'Proof verification is temporarily unavailable.');
  } catch (error) {
    if (error.retryAfter) {
      res.setHeader('Retry-After', String(error.retryAfter));
    }
    res.status(error.statusCode || 502).json({
      ok: false,
      status: error.statusCode === 504 ? 'timeout' : 'error',
      error: proxyErrorMessage(error, 'Proof verification is temporarily unavailable.')
    });
  }
}
...
export async function requestProofConversion(payload) {
  const normalizedPayload = payload && typeof payload === 'object' ? payload : {};
  const language = normalizeLanguageKey(normalizedPayload.language);
  const code = typeof normalizedPayload.code === 'string' ? normalizedPayload.code : '';
  ...
  const conversionPayload = {
    ...
    code,
    fileName: normalizedPayload.fileName || null,
    format: normalizedPayload.format || 'cic-v1',
    verify: true
  };
  ...
  if (!isExecutionConfigured()) {
    ...
    return runProofTaskWithLimit(
      () => runProofConversionLocally(conversionPayload),
      { kind: `${language}-trusted-proof-conversion` }
    );
  }
  ...
  const upstream = await sendRemoteRequest('execution', '/api/proof-convert', {
    method: 'POST',
  ...
  if (!upstream.ok || !result) {
    const error = new Error(extractRemoteError(body, upstream.text || 'Trusted proof conversion failed.'));
...
export async function proxyDistributedCheck(req, res) {
  ...
  try {
    assertProofRequestAllowed(req, '/api/helper/check', { body: req.body || {} });
    await assertDistributedRateLimit(req, { route: 'proof-check', limit: 20, windowSeconds: 60 });
    await assertDistributedRateLimit(req, {
      route: 'proof-check-global',
      includeClientAddress: false,
...
export async function proxyDistributedHelperOperation(req, res, targetPath) {
  ...
  try {
    ...
    const payload = req.body && typeof req.body === 'object'
      ...
      : { verify: true };
    const upstream = await requestDistributedOperation(targetPath, payload, req);
    const responsePayload = upstream.payload && typeof upstream.payload === 'object'
      ? { ...upstream.payload }
      : upstream.payload;
    const jobId = responsePayload && responsePayload.job && responsePayload.job.id;
    if (jobId) {
      responsePayload.jobToken = issueJobCapability(jobId);
    }
    const publicPayload = NODE_ENV === 'production' && upstream.status >= 500
      ? { success: false, error: 'Proof conversion is temporarily unavailable.' }
      : responsePayload;
    res.status(upstream.status).json(withRequestId(publicPayload, req));
  } catch (error) {
    if (error.retryAfter) {
      res.setHeader('Retry-After', String(error.retryAfter));
    }
    res.status(error.statusCode || 502).json(withRequestId({
      success: false,
      error: proxyErrorMessage(error, 'Proof conversion is temporarily unavailable.')
    }, req));
  }
}

function publicRuntimeAvailability(runtimes) {
  const source = runtimes && typeof runtimes === 'object' ? runtimes : {};
  const conversion = source.conversionRuntimes && typeof source.conversionRuntimes === 'object'
    ? source.conversionRuntimes
    : {};
  const family = (name) => {
    const value = conversion[name] && typeof conversion[name] === 'object' ? conversion[name] : {};
    return {
      lean: { available: Boolean(value.lean && value.lean.available) },
      coq: { available: Boolean(value.coq && value.coq.available) }
    };
  };
  return {
    conversionRuntimes: {
      typedLambda: family('typedLambda'),
      cic: family('cic')
    }
  };
}

function sanitizeProductionHelperInfo(payload) {
  if (NODE_ENV !== 'production') return payload;
  const capabilities = payload && payload.capabilities && typeof payload.capabilities === 'object'
    ? Object.fromEntries(
        Object.entries(payload.capabilities)
          .filter(([, value]) => typeof value === 'boolean')
      )
    : {};
  const persistence = payload && payload.vercelPersistence && typeof payload.vercelPersistence === 'object'
    ? payload.vercelPersistence
    : {};
  return {
    ok: Boolean(payload && payload.ok),
    executionConfigured: Boolean(payload && payload.executionConfigured),
    capabilities,
    vercelPersistence: {
      supabaseConfigured: Boolean(persistence.supabaseConfigured),
      urlConfigured: Boolean(persistence.urlConfigured),
      serviceRoleConfigured: Boolean(persistence.serviceRoleConfigured)
    },
    runtimes: publicRuntimeAvailability(payload && payload.runtimes)
  };
}
...
export async function proxyCompositeHelperInfo(req, res) {
  ...
  try {
    ...
    const exposeDeploymentUrls = NODE_ENV !== 'production';
    ...
    if (!upstream.payload || typeof upstream.payload !== 'object') {
      if (NODE_ENV === 'production') {
        res.status(upstream.status >= 400 ? upstream.status : 502).json({
          ok: false,
          error: 'Proof helper is temporarily unavailable.'
        });
        return;
      }
      res.status(upstream.status);
    ...
    const payload = {
      ...
      deployment: {
        ...
        helperServer: exposeDeploymentUrls ? (HELPER_API_BASE_URL || null) : null,
        helperServers: exposeDeploymentUrls ? activeHelperBaseUrls : [],
        helperPrimaryServers: exposeDeploymentUrls ? HELPER_API_BASE_URLS : [],
        helperStandbyServers: exposeDeploymentUrls ? HELPER_STANDBY_BASE_URLS : [],
        executionServer: exposeDeploymentUrls ? (effectiveExecutionBaseUrl || null) : null,
        executionServers: exposeDeploymentUrls ? activeExecutionBaseUrls : [],
      ...
      remotePools: {
        helper: exposeDeploymentUrls ? getRemotePoolSnapshot('helper') : [],
        execution: exposeDeploymentUrls ? getRemotePoolSnapshot('execution') : []
      },
      autoscaling: {
        ...
        burstActiveUntil: HELPER_AUTOSCALE_STATE.burstActiveUntil
          ...
          : null,
        standbyServers: exposeDeploymentUrls ? HELPER_STANDBY_BASE_URLS : [],
    ...
    res.status(upstream.status).json(withRequestId(sanitizeProductionHelperInfo(payload), req));
  } catch (error) {
    res.status(error.statusCode || 502).json(withRequestId({
      success: false,
      error: NODE_ENV === 'production'
        ? 'Proof helper is temporarily unavailable.'
        : error.message
    }, req));
  }
}
