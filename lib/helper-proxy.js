import { runProofConversionLocally } from './proof-convert.js';
...
async function requestDistributedOperation(targetPath, payload) {
  ...
  const proofCheck = await requestProofCheck(normalizedPayload);
  ...
  if (!helperResponse.ok || !helperPayloadResult || !helperPayloadResult.result) {
    if (shouldFallbackToLocalConversion(helperPayloadResult, helperResponse.text)) {
      const result = await buildLocalConversionFallback(normalizedPayload, proofCheck);
      return {
  ...
  return {
    ok: Boolean(mergedResult.ok),
    status: statusCodeForResult(mergedResult),
    payload: {
      success: Boolean(mergedResult.ok),
      result: mergedResult
    }
  };
}
