...
const STORAGE_KEEP_CONCURRENCY = 4;
...
function getIdentityAliases(identity) {
  const aliases = [];
  const seen = new Set();
...
async function getAccountIdentity(req, supabase) {
  ...
  if (google && google.authenticated) {
    ...
    const identity = {
      authenticated: true,
      accountProvider: email ? 'email' : 'google',
      accountId: email || safeString(google.accountId || google.name, 'Google account'),
      email,
      name: safeString(google.name)
    };
    return {
      ...identity,
      accountAliases: getIdentityAliases(identity)
    };
  }

  const emailIdentity = getEmailIdentity(req);
  if (emailIdentity && emailIdentity.authenticated) {
    const email = normalizeEmail(emailIdentity.email || emailIdentity.accountId);
    const identity = {
      authenticated: true,
      accountProvider: 'email',
      accountId: email,
      email,
      name: safeString(emailIdentity.name)
    };
    return {
      ...identity,
      accountAliases: getIdentityAliases(identity)
  ...
  if (blue && blue.authenticated) {
    return {
      ...blue,
      accountAliases: getIdentityAliases(blue)
    };
  }
  return {
    authenticated: false,
...
async function listStorageKeeps(supabase, identity) {
  await ensureSearchKeepBucket(supabase);
  const aliases = getIdentityAliases(identity);
  const prefixes = (aliases.length ? aliases : [identity]).map(getStoragePrefix);
  const listings = await mapWithConcurrency(prefixes, STORAGE_KEEP_CONCURRENCY, async (prefix) => {
    ...
    return (Array.isArray(listed.data) ? listed.data : [])
      ...
      .filter(({ file }) => {
        ...
        return Boolean(name && name.endsWith('.json'));
      });
  });
  const storedFiles = listings.flat();
  const keeps = await mapWithConcurrency(storedFiles, STORAGE_KEEP_CONCURRENCY, async ({ file, prefix }) => {
    const name = safeString(file && file.name);
    const storagePath = `${prefix}/${name}`;
    try {
      const payload = await downloadStorageKeep(supabase, storagePath);
      return normalizeStoredKeep(storagePath, payload, file);
    } catch (error) {
      // Skip unreadable records instead of failing the whole keep drawer.
      return null;
    }
  });
  return keeps.filter(Boolean);
}
...
async function listDbKeeps(supabase, identity) {
  const aliases = getIdentityAliases(identity);
  const hashes = (aliases.length ? aliases : [identity]).map((alias) => alias.accountIdHash || getIdentityHash(alias)).filter(Boolean);
  const { data, error } = await supabase
    .from('search_chat_keeps')
    .select('id, title, system_mode, turn_count, citation_count, conversation, created_at, updated_at, account_provider, account_id_hash')
    .in('account_id_hash', hashes.length ? hashes : [getIdentityHash(identity)])
    .order('updated_at', { ascending: false })
    .limit(MAX_KEEP_LIST);
  if (error) throw error;
  const allowed = new Set(hashes.length ? hashes : [getIdentityHash(identity)]);
  return Array.isArray(data)
    ? data.filter((row) => allowed.has(safeString(row && row.account_id_hash))).map(mapDbKeep)
    : [];
}
...
async function listKeeps({ supabase, identity }) {
  ...
  const [dbResult, storageResult] = await Promise.allSettled([
    listDbKeeps(supabase, identity),
    listStorageKeeps(supabase, identity)
  ]);
  if (dbResult.status === 'fulfilled') {
    dbKeeps = dbResult.value;
  } else {
  ...
  if (storageResult.status === 'fulfilled') {
    storageKeeps = storageResult.value;
  } else if (!dbKeeps.length) {
...
async function deleteKeep({ supabase, identity, keepId }) {
  ...
  if (isStorageKeepId(id)) {
    const aliases = getIdentityAliases(identity);
    const prefixes = (aliases.length ? aliases : [identity]).map((alias) => `${getStoragePrefix(alias)}/`);
...
export async function sendSearchChatKeepResponse(req, res) {
  ...
  try {
    ...
    res.status(status).json({
      ok: false,
      saved: false,
      error: getPublicErrorMessage(error, 'Could not keep this chat.', status)
    });
  }
}
